...
<!---- ITEM HEADLINE --->
  ...
  <div>
    ...
    >
      ...
      <v-toolbar-title>Polyimide Tape</v-toolbar-title>
...
export default {
  ...
  methods: {
    load() {
      this.loadData(1,20,"","")
      if(!this.isAdmin) {
        API.getBrandsList("Polyimide Tape").then(brands => {
          brands.unshift("");
    ...
    async save() {
      ...
      try {
        ...
        this.products.map(product => {
          writeProductData('Polyimide Tape', product)
    ...
    loadData(page,pageNums,searchBrand,searchText) {
      this.loading = true
      API.getPolyimideTape(page,pageNums,searchBrand,searchText).then(products => {
        if(this.isAdmin) {
