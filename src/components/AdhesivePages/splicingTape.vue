...
<!---- ITEM HEADLINE --->
  ...
  <div>
    ...
    >
      ...
      <v-toolbar-title>Splicing Tape</v-toolbar-title>
...
export default {
  ...
  methods: {
    load() {
      this.loadData(1,20,"","")
      if(!this.isAdmin) {
        API.getBrandsList("Splicing Tape").then(brands => {
          brands.unshift("");
    ...
    async save() {
      ...
      try {
        ...
        this.products.map(product => {
          writeProductData('Splicing Tape', product)
    ...
    loadData(page,pageNums,searchBrand,searchText) {
      this.loading = true
      API.getSplicingTape(page,pageNums,searchBrand,searchText).then(products => {
        if(this.isAdmin) {
