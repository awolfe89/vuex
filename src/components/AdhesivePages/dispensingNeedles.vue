...
<!---- ITEM HEADLINE --->
  ...
  <div>
    ...
    >
      ...
      <v-toolbar-title>Dispensing Needles</v-toolbar-title>
...
export default {
  ...
  methods: {
    load() {
      this.loadData(1,20,"","")
      if(!this.isAdmin) {
        API.getBrandsList("Dispensing Needles").then(brands => {
          brands.unshift("");
    ...
    async save() {
      ...
      try {
        ...
        this.products.map(product => {
          writeProductData('Dispensing Needles', product)
    ...
    loadData(page,pageNums,searchBrand,searchText) {
      this.loading = true
      API.getDispensingNeedles(page,pageNums,searchBrand,searchText).then(products => {
        if(this.isAdmin) {
